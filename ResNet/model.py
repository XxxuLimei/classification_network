...
# define residual block, for ResNet-18/34
class BasicBlock(nn.Module):
    expansion = 1 # if the num_conv varies, then this will not equals to 1

    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        super(BasicBlock, self).__init__()
        # when stride equals to 1, it is real line structure; otherwise it is dotted line structure
...
# define the residual block for ResNet-50/101/152    
class BottleNeck(nn.Module):
    expansion = 4 # layer3's kernel_num equals to layer1/2's kernel_num * 4
    ...
    def __init__(self, in_channel, out_channel, stride=1, downsample=None):
        ...
        self.downsample = downsample # downsample function
...
class ResNet(nn.Module):
    def __init__(self, block, block_num, num_classes=1000, include_top=True):
        ...
        if self.include_top:
            self.avgpool = nn.AdaptiveAvgPool2d((1, 1)) # output's H and W equal (1, 1)
